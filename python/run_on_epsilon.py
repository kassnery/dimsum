...
try:
    ...
    try:
        speeds = cPickle.load(
            open("../old_results/{} {} Runs Gamma 4 Single Run.pkl".format(dataset,RUNS),'rb')
        )
        total = 0
        actual_size = 0
        for x in open(target_file):
            total += int(x.split()[1])
            if total >= 0x7fffffff:
                    break
            actual_size+=1
        computed_size = 10000000
        real_speeds = [{
            algorithm: [x/computed_size*actual_size for x in
                        speed[algorithm]] for algorithm in speed}
                        for speed in speeds]
        speeds = real_speeds
    except:        
        speeds = []
...
def plot_speeds(speeds, dataset):
    ...
    plt.plot(phis, average_speeds["CMH"],"-^",label="CM")
    plt.plot(phis, average_speeds["CCFC"],"-v",label="CS")
    ...
    plt.tight_layout()
    fig = plt.gcf()
    fig.canvas.set_window_title("{} {} Runs Gamma 4 Single Run".format(
        dataset,RUNS))
