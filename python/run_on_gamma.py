...
shows_legend = True if raw_input("Show legend?(y/n)")=='y'\
               else False
...
def plot_speeds(speeds, dataset, shows_legend = False):
    ...
    for algorithm in speeds[0].keys():
            average_speeds[algorithm] = [
                sum([speed[algorithm][i] for speed in speeds])
                /len(speeds)
    ...
    window_title = "Gammas {} {} Runs Single Run".format(
        dataset,len(speeds)).replace(".","-")
    if shows_legend:
        plt.legend(loc="center")
        window_title = "gamma_legend"
    plt.ylim(ymin=0)
    plt.tight_layout()
    fig = plt.gcf()
...
plot_speeds(speeds,dataset, shows_legend=shows_legend)
